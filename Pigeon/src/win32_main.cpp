	...
	HANDLE& logFile)
{
	...
	// Enforce single instance
	{
		...
		// NOTE: This isn't necessarily going to work aross versions if the single instance logic
		...
		if (GetLastError() == ERROR_ALREADY_EXISTS)
		{
			...
			if (uResult == WAIT_FAILED)
			{
				...
				// TODO: Pretty sure this is wrong. Should destroy the mutex
				 ...
				 || (newProcessStartTime == startTime && newProcessID > processID))
				{
					if (singleInstanceMutex)
					{
						...
						if (success)
						{
							...
							// TODO: Pretty sure this is wrong (probably a hack so we don't try to release a second time)
							// Also we could try to free resources a second time
							singleInstanceMutex = nullptr;
						}
					}
